...
type PackagingCreateAdminType = {
    visible: boolean,
    onClose: () => void,
    add: (item: PackagingDto) => void,
    showToast: React.Ref<Toast>,
    list: PackagingDto[],
    service: PackagingAdminService,
    t: TFunction
}
const Create: React.FC<PackagingCreateAdminType> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const emptyItem = new PackagingDto();
        const {
            ...
            saveItem,
            formateDate
        } = useCreateHook<PackagingDto, PackagingCriteria>({list, emptyItem, onClose, add, showToast,service, isFormValid})
            ...
            <TabPanel header={t("packaging.tabPan")}>
                <div className="formgrid grid">
                    <div className="field col-6">
                        ...
                        <InputText id="name" value={item.name} onChange={(e) => onInputTextChange(e, 'name')} required className={classNames({'p-invalid': submitted && !item.name})} />
                    ...
                    <div className="field col-6">
                        ...
                        <Calendar id="dateStart" value={item.dateStart} onChange={(e) => onInputDateChange(e, 'dateStart')} dateFormat="dd/mm/yy"  showIcon={true} />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="dateEnd">{t("packaging.dateEnd")}</label>
                        <Calendar id="dateEnd" value={item.dateEnd} onChange={(e) => onInputDateChange(e, 'dateEnd')} dateFormat="dd/mm/yy"  showIcon={true} />
                    </div>
                    <div className="field col-6">
                    ...
                    <div className="field col-6">
                        ...
                        <InputNumber id="maxEntity" value={item.maxEntity} onChange={(e) => onInputNumerChange(e, 'maxEntity')} />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="maxProjet">{t("packaging.maxProjet")}</label>
                        <InputNumber id="maxProjet" value={item.maxProjet} onChange={(e) => onInputNumerChange(e, 'maxProjet')} />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="maxAttribut">{t("packaging.maxAttribut")}</label>
                        <InputNumber id="maxAttribut" value={item.maxAttribut} onChange={(e) => onInputNumerChange(e, 'maxAttribut')} />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="maxIndicator">{t("packaging.maxIndicator")}</label>
                        <InputNumber id="maxIndicator" value={item.maxIndicator} onChange={(e) => onInputNumerChange(e, 'maxIndicator')} />
                    </div>
                    <div className="field col-6">
                        <label htmlFor="categoryPackaging">{t("packaging.categoryPackaging")}</label>
                        <Dropdown  id="categoryPackagingDropdown"  value={item.categoryPackaging} options={categoryPackagings} onChange={(e) => onDropdownChange(e, 'categoryPackaging')}   placeholder={t("packaging.categoryPackagingPlaceHolder")} filter filterPlaceholder={t("packaging.categoryPackagingPlaceHolderFilter")} optionLabel="libelle" showClear/>
                    </div>
                </div>
            </TabPanel>
        </TabView>
