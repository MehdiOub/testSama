...
import Edit from '@/components/admin/view/template/technology/edit/technology-edit-admin.component';
import Create from '@/components/admin/view/template/technology/create/technology-create-admin.component';
import View from '@/components/admin/view/template/technology/view/technology-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new TechnologyCriteria();
    const service = new TechnologyAdminService();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<TechnologyDto, TechnologyCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("Technology")
        fetchItems(criteria);
    }, []);
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("technology.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("technology.code")}</label>
                                <InputText id="1" value={criteria.code} onChange={(e) => setCriteria({ ...criteria, code: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("technology.libelle")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("technology.code")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("technology.libelle")} sortable></Column>
                    
                    
                    <Column field="logo" header={t("technology.logo")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("technology.deleteTechnologyConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("technology.deleteTechnologysConfirmationMessage")}</span>}
                </div>
            </Dialog>
