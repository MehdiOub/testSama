...
import Edit from '@/components/admin/view/packaging/category-packaging/edit/category-packaging-edit-admin.component';
import Create from '@/components/admin/view/packaging/category-packaging/create/category-packaging-create-admin.component';
import View from '@/components/admin/view/packaging/category-packaging/view/category-packaging-view-admin.component';
...
const List = () => {
    ...
    const emptyItem = new CategoryPackagingDto();
    const emptyCriteria = new CategoryPackagingCriteria();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<CategoryPackagingDto, CategoryPackagingCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("CategoryPackaging")
        fetchItems(criteria);
    }, []);
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("categoryPackaging.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("categoryPackaging.code")}</label>
                                <InputText id="1" value={criteria.code} onChange={(e) => setCriteria({ ...criteria, code: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("categoryPackaging.libelle")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("categoryPackaging.code")} sortable></Column>
                    
                    
                    <Column field="libelle" header={t("categoryPackaging.libelle")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("categoryPackaging.deleteCategoryPackagingConfirmationMessage")} <b>{item.libelle}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("categoryPackaging.deleteCategoryPackagingsConfirmationMessage")}</span>}
                </div>
            </Dialog>
