...
import ma.zs.zyn.service.facade.admin.collaborator.PaimentCollaboratorAdminService;
import ma.zs.zyn.ws.converter.collaborator.PaimentCollaboratorConverter;
...
public class PaimentCollaboratorRestAdmin {
    ...
    @Operation(summary = "Finds a paimentCollaborator by libelle")
    @GetMapping("libelle/{libelle}")
    public ResponseEntity<PaimentCollaboratorDto> findByLibelle(@PathVariable String libelle) {
	    PaimentCollaborator t = service.findByReferenceEntity(new PaimentCollaborator(libelle));
        if (t != null) {
            converter.init(true);
            PaimentCollaboratorDto dto = converter.toDto(t);
            return getDtoResponseEntity(dto);
