...
public class InscriptionCollaboratorRestCollaborator {
    ...
    @Operation(summary = "Finds an optimized list of all inscriptionCollaborators")
    @GetMapping("optimized")
    public ResponseEntity<List<InscriptionCollaboratorDto>> findAllOptimized() throws Exception {
        ResponseEntity<List<InscriptionCollaboratorDto>> res = null;
        List<InscriptionCollaborator> list = service.findAllOptimized();
        HttpStatus status = HttpStatus.NO_CONTENT;
        converter.initObject(true);
        List<InscriptionCollaboratorDto> dtos  = converter.toDto(list);
        if (dtos != null && !dtos.isEmpty())
    ...
    @Operation(summary = "Finds a inscriptionCollaborator by reference")
    @GetMapping("reference/{reference}")
    public ResponseEntity<InscriptionCollaboratorDto> findByReference(@PathVariable String reference) {
	    InscriptionCollaborator t = service.findByReferenceEntity(new InscriptionCollaborator(reference));
        if (t != null) {
            converter.init(true);
            InscriptionCollaboratorDto dto = converter.toDto(t);
            return getDtoResponseEntity(dto);
