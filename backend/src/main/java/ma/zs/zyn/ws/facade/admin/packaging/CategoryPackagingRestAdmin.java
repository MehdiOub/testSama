...
import ma.zs.zyn.ws.converter.packaging.CategoryPackagingConverter;
import ma.zs.zyn.ws.dto.packaging.CategoryPackagingDto;
...
public class CategoryPackagingRestAdmin {
    ...
    @Operation(summary = "Finds a categoryPackaging by libelle")
    @GetMapping("libelle/{libelle}")
    public ResponseEntity<CategoryPackagingDto> findByLibelle(@PathVariable String libelle) {
	    CategoryPackaging t = service.findByReferenceEntity(new CategoryPackaging(libelle));
        if (t != null) {
            CategoryPackagingDto dto = converter.toDto(t);
            return getDtoResponseEntity(dto);
        }
